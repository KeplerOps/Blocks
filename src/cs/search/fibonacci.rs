...
/// # Examples
/// ```
/// use blocks_cs_search::algorithms::fibonacci;
///
/// let numbers = vec![1, 2, 3, 4, 5, 6];
/// assert_eq!(fibonacci::search(&numbers, &4), Ok(Some(3)));
/// assert_eq!(fibonacci::search(&numbers, &7), Ok(None));
/// ```
///
/// # Performance
/// * Time: O(log n)
...
pub fn search<T: Ord>(data: &[T], target: &T) -> Result<Option<usize>> {
    ...
    if !is_sorted(data) {
        return Err(SearchError::InvalidInput(
            "Fibonacci search requires sorted input".to_string(),
        ));
    }
...
mod tests {
    ...
    fn test_fibonacci_sequence_lengths() {
        ...
        assert_eq!(search(&data1, &1), Ok(Some(1)));
        assert_eq!(search(&data2, &3), Ok(Some(3)));
        assert_eq!(search(&data3, &5), Ok(Some(5)));
        assert_eq!(search(&data4, &8), Ok(Some(8)));
    }
    ...
    fn test_non_fibonacci_lengths() {
        ...
        assert_eq!(search(&data2, &4), Ok(Some(4)));
        assert_eq!(search(&data3, &6), Ok(Some(6)));
    }
}